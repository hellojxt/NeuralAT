...
row_layout = Layout(display="flex", flex_flow="row", justify_content="space-around")
col_layout = Layout(
    display="flex", flex_flow="column", align_items="stretch", flex="1 1 auto"
)
...
class viewer:
    def __init__(
        ...
        if self.data is None:
            ...
            self.items = [self.init_data_selector(), self.init_3D()]

    @property
    def box(self):
        return Box(self.items, layout=col_layout)
    ...
    def init_3D(self):
        ...
        if self.show_axis:
            ...
            scene = dict(
                xaxis=dict(
                    showticklabels=False, visible=False, range=[bound_min, bound_max]
                ),
                yaxis=dict(
                    showticklabels=False, visible=False, range=[bound_min, bound_max]
                ),
        ...
        self.fig.update_layout(title_font_size=50)
        return Box([self.fig], layout=row_layout)

    def init_data_selector(self):
        self.int_range = IntSlider(
            min=0,
            max=len(self.data) - 1,
            step=1,
            value=0,
            layout=Layout(flex="3 1 auto"),
