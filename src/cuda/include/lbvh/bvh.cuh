...
void construct_internal_nodes(const basic_device_bvh<Real, dim, Object, IsConst> &self,
...
class bvh
{
    ...
    public:
        ...
        bvh(InputIterator first, InputIterator last, bool query_host_enabled = false)
            : objects_d_(first, last), query_host_enabled_(query_host_enabled)
        {
            this->construct();
        }
        ...
        bvh_device<real_type, dim, object_type> get_device_repr() noexcept
        {
            return bvh_device<real_type, dim, object_type>{
                static_cast<unsigned int>(nodes_.size()), static_cast<unsigned int>(objects_d_.size()),
        ...
        void construct()
        {
            ...
            thrust::transform(this->objects_d_.begin(), this->objects_d_.end(), aabbs_.begin() + num_internal_nodes,
                              aabb_getter_type());
            ...
            thrust::transform(this->objects_d_.begin(), this->objects_d_.end(), aabbs_.begin() + num_internal_nodes,
                              morton.begin(), morton_code_calculator_type(aabb_whole));
            ...
            thrust::copy(thrust::make_counting_iterator<index_type>(0),
                         thrust::make_counting_iterator<index_type>(num_objects), indices.begin());
            ...
            {
                thrust::transform(morton.begin(), morton.end(), indices.begin(), morton64.begin(),
            ...
            thrust::transform(indices.begin(), indices.end(), this->nodes_.begin() + num_internal_nodes,
            ...
            thrust::for_each(thrust::device, thrust::make_counting_iterator<index_type>(num_internal_nodes),
