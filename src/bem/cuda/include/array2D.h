...
class GArr2D
{
    public:
        ...
        GArr2D(CArr2D<T> &A) { this->assign(A); }
        ...
        inline GArr<T> operator[](unsigned int id) { return GArr<T>(begin() + id * cols, cols); }

        inline const GArr<T> operator[](unsigned int id) const { return GArr<T>(begin() + id * cols, cols); }

        CGPU_FUNC inline const T *begin() const { return data.begin(); }
        CGPU_FUNC inline T *begin() { return data.begin(); }

        inline CArr2D<T> cpu() { return CArr2D<T>(*this); }
...
class CArr2D
{
    public:
        ...
        CArr2D(int rows, int cols) { this->resize(rows, cols); }
        ...
        CArr2D(GArr2D<T> &A) { this->assign(A); }
        ...
        inline const T &operator()(const uint i, const uint j) const { return data[i * cols + j]; }

        inline T &operator()(const uint i, const uint j) { return data[i * cols + j]; }

        inline CArr<T> operator[](unsigned int id) { return CArr<T>(begin() + id * cols, cols); }

        inline const CArr<T> operator[](unsigned int id) const { return CArr<T>(begin() + id * cols, cols); }

        inline int index(const uint i, const uint j) const { return i * cols + j; }
        ...
        inline GArr2D<T> gpu() { return GArr2D<T>(*this); }
