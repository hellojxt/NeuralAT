...
#define MLOG_ERROR(x)                                                          \
  MLOG_FILE << MLOG_ERROR_COLOR << x << MLOG_RESET_COLOR << std::endl;
#define MLOG_WARNING(x)                                                        \
  MLOG_FILE << MLOG_WARNING_COLOR << x << MLOG_RESET_COLOR << std::endl;
#define MLOG_INFO(x)                                                           \
  MLOG_FILE << MLOG_INFO_COLOR << x << MLOG_RESET_COLOR << std::endl;
#define MLOG_DEBUG(x)                                                          \
  MLOG_FILE << MLOG_DEBUG_COLOR << x << MLOG_RESET_COLOR << std::endl;
#define MLOG_LINE(x)                                                           \
  MLOG_FILE << MLOG_BOLD_COLOR << "---------------" << x << "---------------"  \
            << MLOG_RESET_COLOR << std::endl;
...
#define TOCK_INLINE(x)                                                         \
  ...
  MLOG_FILE << #x ": "                                                         \
            << std::chrono::duration_cast<std::chrono::duration<double>>(      \
                   std::chrono::steady_clock::now() - bench_inline_##x)        \
                   .count()                                                    \
            << "s"                                                             \
            << "  \t";                                                         \
  std::cout.precision(ss_##x);
...
/**
 * @brief Macro definition for execuation of cuda kernels, note that at lease
...
public:
  ...
  Range() : start(0), end(0) {}
  inline int length() { return end - start; }
